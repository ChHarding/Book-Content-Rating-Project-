...
def get_book_content_warning(title, author=""):
    ...
    google_title, google_authors, google_description, _, _ = search_google_books(title, author)   # Adjusted to accept five return values
...
def show_book_image(url):
    ...
    try:
        ...
        image.thumbnail((100, 150), Image.ANTIALIAS)  # Resizing the image
...
def search_book_ratings():
    ...
    _, google_authors, _, purchase_link, thumbnail_url = search_google_books(book_title)

# Format the authors list into a readable string
    author_name = ', '.join(google_authors) if google_authors else "Unknown"
    ...
    if thumbnail_url:
        show_book_image(thumbnail_url)

    if purchase_link:
        purchase_button = tk.Button(result_frame, text="Purchase Book", command=lambda: open_link(purchase_link))
