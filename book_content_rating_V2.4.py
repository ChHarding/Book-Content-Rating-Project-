...
def search_google_books(title, author=""):
    ...
    if response.status_code == 200:
        ...
        if "items" in data and len(data["items"]) > 0:
            ...
            # Grab title and author of what Google thinks is the best match
            author_name_lst = data["items"][0]["volumeInfo"].get("authors", [])
            title_ = data["items"][0]["volumeInfo"]["title"] 
            desc = data["items"][0]["volumeInfo"].get("description", "")
            return title_, author_name_lst, desc
...
def get_book_content_warning(title, author=""):
    ...
    google_title, google_authors, google_description = search_google_books(title, author)
    ...
    if combined_description == "":
        return 0, "No description found for this book."
...
# Function that fetches the author(s) information
...
def search_book_ratings():
    ...
    rating, reason = get_book_content_warning(book_title)
    
    # Fetching author information
    _, google_authors, _ = search_google_books(book_title)
    _, open_books_authors, _ = search_open_books(book_title)
    author_name = get_author_info(google_authors, open_books_authors)

    # If no rating is found and no authors are found, assume the book wasn't found
    if not rating and not author_name:
        messagebox.showerror("Error", "No results found for the given book title. Please try again with a different title.")
        return

    # Error handling: check if the title is empty
