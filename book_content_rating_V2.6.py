...
def get_book_info(title, author=""):
    """
    Fetches book descriptions using Google Books and Open Books APIs, and returns a combined list of the top 5 unique books.
    """
    google_books = search_google_books(title, author)
    open_books = search_open_books(title, author)
    combined_books = google_books + open_books

    # Remove duplicates based on title and author, and limit to top 5 unique results
    seen = set()
    unique_books = []
    for book in combined_books:
        identifier = (book['title'], book['author'])
        if identifier not in seen and len(unique_books) < 5:
            seen.add(identifier)
...
def display_selected_book(book):
    ...
    if 'image_url' in book and book['image_url']:
        show_book_image(book['image_url'])
